...
resource "hcloud_server" "database" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "$JOIN_CMD",
      "ssh -o StrictHostKeyChecking=no root@${hcloud_server.master[0].ipv4_address} 'kubectl label nodes k8s-database-${count.index + 1} node-role.kubernetes.io/worker=worker'",
      "ssh -o StrictHostKeyChecking=no root@${hcloud_server.master[0].ipv4_address} 'kubectl label nodes k8s-database-${count.index + 1} role=database'"
    ]
  }
}